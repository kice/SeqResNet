...
from ModelBottleneckDropOutPatched import ResNetCIFAR
import numpy as np
import random
...
def demo(data, save, valid_size=0, n_epochs=300, batch_size=64, seed=None):
    ...
    # Models
    model = ResNetCIFAR(Classes=100, BlocksPerStage=[3,3,3], PyramidFactor=[1,2,3], Widening=12, Granularity=32, BatchNormalization=True, WeightScale=0)

    train(model=model, train_set=train_set, test_set=test_set, save=save,
          valid_size=valid_size, n_epochs=n_epochs, batch_size=batch_size, lr=0.1, seed=seed, info='C100P6.0BottleneckDropoutPatched')
