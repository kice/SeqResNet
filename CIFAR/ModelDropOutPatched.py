...
class SequentialConvolutionalLayer(nn.Module):
      def __init__(self, InputChannels, OutputChannels, ReceptiveField=3, Windowed=True, BatchNormalization=True, Junction=False, WeightScale=0, k=8, DropRate=0.1):
          ...
          for x in range(OutputChannels // k):
              Filters += [ConvolutionalLayer(InputChannels if Windowed else InputChannels + x * k, k, ReceptiveField=ReceptiveField, Strides=1, Activation=True, BatchNormalization=BatchNormalization, Junction=Junction, WeightScale=WeightScale)]
...
summary(ResNetCIFAR(Classes=10, BlocksPerStage=[2,2,2], PyramidFactor=[1,2,3], Widening=7, Granularity=16, BatchNormalization=True, WeightScale=0),(3,32,32))        