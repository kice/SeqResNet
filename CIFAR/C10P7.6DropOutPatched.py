...
def demo(data, save, valid_size=0, n_epochs=300, batch_size=64, seed=None):
    ...
    # Models
    model = ResNetCIFAR(Classes=10, BlocksPerStage=[1,1,1], PyramidFactor=[1,2,3], Widening=10, Granularity=16, BatchNormalization=True, WeightScale=0)

    train(model=model, train_set=train_set, test_set=test_set, save=save,
          valid_size=valid_size, n_epochs=n_epochs, batch_size=batch_size, lr=0.1, seed=seed, info='C10P7.6DropOutPatched')
