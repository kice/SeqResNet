...
def PulversizeAggregatedFeatures(ConsolidatedActivatedFeatures, ConsolidatedRawFeatures, GroupNumber):
    RawFeatures = []
    ActivatedFeatures = []
    GroupSize = ConsolidatedActivatedFeatures.size()[1] // GroupNumber  
    for x in range(GroupNumber):
        RawFeatures += [ConsolidatedRawFeatures[:, x * GroupSize : (x + 1) * GroupSize, :, :]]
        ActivatedFeatures += [ConsolidatedActivatedFeatures[:, x * GroupSize : (x + 1) * GroupSize, :, :]]
    return RawFeatures, ActivatedFeatures

def CheckpointedForward(Action, ListOfInputs):
    def AuxiliaryAction(*x):
        x = torch.cat(x, 1) if len(x) > 1 else x[0]
        RawFeatures, ActivatedFeatures = Action(x)
        return RawFeatures, ActivatedFeatures
    if any(z.requires_grad for z in ListOfInputs):
       Parameters = list(Action.LinearLayer.parameters())
       Parameters += list(Action.BatchNormalizationLayer.parameters())
       Arguements = ListOfInputs + Parameters
       RawFeatures, ActivatedFeatures = cp.CheckpointFunction.apply(AuxiliaryAction, len(ListOfInputs), *Arguements)
    else:
       RawFeatures, ActivatedFeatures = AuxiliaryAction(*ListOfInputs)
    return RawFeatures, ActivatedFeatures
...
class SequentialConvolutionalLayer(nn.Module):
      def __init__(self, InputChannels, OutputChannels, ReceptiveField=3, Windowed=True, BatchNormalization=True, Junction=False, WeightScale=0, k=8):
          ...
          for x in range(OutputChannels // k):
              BottleneckLayers += [ConvolutionalLayer(InputChannels if Windowed else InputChannels + x * k, k, 1, 1, BatchNormalization=BatchNormalization)]
              TransformationLayers += [ConvolutionalLayer(k, k, ReceptiveField, 1, True, BatchNormalization, Junction, WeightScale)]  
          self.BottleneckLayers = nn.ModuleList(BottleneckLayers)
          self.TransformationLayers = nn.ModuleList(TransformationLayers)
      ...
      def forward(self, x, Shortcut=None):
          ...
          RawSequence = []
          ActivatedSequence = []
          for y in range(len(self.BottleneckLayers)):
              CurrentShortcutSlice = Shortcut[y] if Shortcut is not None else None
              _, ActivatedFeatures = CheckpointedForward(self.BottleneckLayers[y], SequenceView)  
              RawFeature, ActivatedFeature = self.TransformationLayers[y](ActivatedFeatures, CurrentShortcutSlice)
              RawSequence += [RawFeature]
              ActivatedSequence += [ActivatedFeature]
              if self.Windowed:
                 SequenceView.pop(0)
              SequenceView += [ActivatedFeature]    
          return RawSequence, ActivatedSequence
...
class DownsampleLayer(nn.Module):
      def __init__(self, InputChannels, OutputChannels, BatchNormalization=True, k=8, Reduce=1):
          ...
          self.GroupNumber = OutputChannels // k // Reduce
          ...
          self.ShrinkingLayer = ConvolutionalLayer(OutputChannels, OutputChannels, 3, 2, BatchNormalization=BatchNormalization, Groups=self.GroupNumber)
      def forward(self, x):
          _, ActivatedFeatures = self.ExtensionLayer(x)
          x += ActivatedFeatures
          ConsolidatedRawFeatures, ConsolidatedActivatedFeatures = CheckpointedForward(self.ShrinkingLayer, x) 
          RawFeatures, ActivatedFeatures = PulversizeAggregatedFeatures(ConsolidatedActivatedFeatures, ConsolidatedRawFeatures, self.GroupNumber)
          return RawFeatures, ActivatedFeatures
...
class ResNetImageNet(nn.Module):
      def __init__(self, Classes=1000, BlocksPerStage=[1, 1, 1, 1], PyramidFactor=[1, 2, 4, 8], Widening=2, Granularity=[8, 16, 32, 64], BatchNormalization=True, WeightScale=0):
          ...
          self.GroupNumberOfHead = 64 * Widening // Granularity[0]
          ...
          self.ShrinkingLayer = ConvolutionalLayer(64 * Widening, 64 * Widening, 3, 2, BatchNormalization=BatchNormalization, Groups=self.GroupNumberOfHead)
      ...
      def forward(self, x):
          ...
          ConsolidatedRawFeatures, ConsolidatedActivatedFeatures = CheckpointedForward(self.ShrinkingLayer, ActivatedFeatures)
          RawFeatures, ActivatedFeatures = PulversizeAggregatedFeatures(ConsolidatedActivatedFeatures, ConsolidatedRawFeatures, self.GroupNumberOfHead)
          ...
          RawFeatures, ActivatedFeatures = CheckpointedForward(self.Blender, ActivatedFeatures)
          x = self.Compress(ActivatedFeatures)
          x = x.view(x.size(0), -1)
          x = self.Classifier(x)
          return x    
      
Model3xLite = ResNetImageNet(Classes=1000, BlocksPerStage=[1, 2, 4, 1], PyramidFactor=[1, 2, 4, 8], Widening=3, Granularity=[32, 64, 64, 128], BatchNormalization=True, WeightScale=0)  
Model4xLite = ResNetImageNet(Classes=1000, BlocksPerStage=[1, 3, 5, 1], PyramidFactor=[1, 2, 4, 8], Widening=4, Granularity=[32, 64, 64, 128], BatchNormalization=True, WeightScale=0)  
Model4xLarge = ResNetImageNet(Classes=1000, BlocksPerStage=[2, 4, 6, 2], PyramidFactor=[1, 2, 4, 8], Widening=4, Granularity=[32, 64, 64, 128], BatchNormalization=True, WeightScale=0) 