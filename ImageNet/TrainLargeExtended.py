...
parser.add_argument('--start-epoch', default=90, type=int, metavar='N',
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            ...
            with open('train_extended.log', 'a') as f:
               print('Epoch: [{0}][{1}/{2}]\t'
                  'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                  'Data {data_time.val:.3f} ({data_time.avg:.3f})\t'
                  'Loss {loss.val:.4f} ({loss.avg:.4f})\t'
            ...
            if i % args.print_freq == 0:
                print('Test: [{0}/{1}]\t'
                      ...
                      'Acc@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                       ...
                       top1=top1, top5=top5))
                with open('validate_extended.log', 'a') as f:
                   print('Test: [{0}/{1}]\t'
                      'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                      'Loss {loss.val:.4f} ({loss.avg:.4f})\t'
                      'Acc@1 {top1.val:.3f} ({top1.avg:.3f})\t'
...
def save_checkpoint(state, is_best, filename='checkpoint_extended.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'model_best_extended.pth.tar')
